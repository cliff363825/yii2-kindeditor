...
class KindEditorUploadAction extends Action
{
    ...
    public function run()
    {
        ...
        if (empty($_FILES) === false) {
            ...
            $save_path = rtrim($this->savePath, '\\/') . '/';
            //创建文件夹
            if ($dir_name !== '') {
                $save_path .= $dir_name . "/";
            }
            $save_path .= rtrim($this->resolveSubPath($this->subPath, $file_name), '\\/') . '/';
            if (!file_exists($base_path . $save_path)) {
                mkdir($base_path . $save_path, 0755, true);
            }
            //检查目录
            if (@is_dir($base_path . $save_path) === false) {
                $this->alert("上传目录不存在。");
            }
            //检查目录写权限
            if (@is_writable($base_path . $save_path) === false) {
                $this->alert("上传目录没有写权限。");
            }
            //新文件名
            $new_file_name = date("YmdHis") . '_' . rand(10000, 99999) . '.' . $file_ext;
            //移动文件
            $file_path = $base_path . $save_path . $new_file_name;
            if (move_uploaded_file($tmp_name, $file_path) === false) {
                $this->alert("上传文件失败。");
            }
            @chmod($file_path, 0644);
            $file_url = $base_url . $save_path . $new_file_name;
